...
import {isSupportedType,isValidType,isEmptyValue,isInChoices,isArray,isDocument,isEmbeddedDocument,isString,isNumber,isDate} from './validate';
        ...
        _.keys(that._schema).forEach(function(key) {
            ...
            if (!isValidType(value, that._schema[key].type)) {
                ...
                throw new ValidationError('Value assigned to ' + that.collectionName() + '.' + key +
                    ' should be ' + typeName + ', got ' + valueName, key);
            }

            if (that._schema[key].required && isEmptyValue(value)) {
                throw new ValidationError('Key ' + that.collectionName() + '.' + key +
                    ' is required' + ', but got ' + value, key);
            }

            if (that._schema[key].match && isString(value) && !that._schema[key].match.test(value)) {
                throw new ValidationError('Value assigned to ' + that.collectionName() + '.' + key +
                    ' does not match the regex/string ' + that._schema[key].match.toString() + '. Value was ' + value, key);
            }

            if (!isInChoices(that._schema[key].choices, value)) {
                throw new ValidationError('Value assigned to ' + that.collectionName() + '.' + key +
                    ' should be in choices [' + that._schema[key].choices.join(', ') + '], got ' + value, key);
            }

            if (isNumber(that._schema[key].min) && value < that._schema[key].min) {
                throw new ValidationError('Value assigned to ' + that.collectionName() + '.' + key +
                    ' is less than min, ' + that._schema[key].min + ', got ' + value, key);
            }

            if (isNumber(that._schema[key].max) && value > that._schema[key].max) {
                throw new ValidationError('Value assigned to ' + that.collectionName() + '.' + key +
                    ' is less than max, ' + that._schema[key].max + ', got ' + value, key);
            }

            let validationResult;
            if (typeof(that._schema[key].validate) === 'function' && (validationResult=that._schema[key].validate(value))!==true && typeof validationResult !== 'undefined') {
              if(validationResult===false) {
                throw new ValidationError('Value assigned to ' + that.collectionName() + '.' + key +
                    ' failed custom validator. Value was ' + value, key);
              }
              else {
                throw new ValidationError(validationResult, key);
              }
            }
        });
    }
